...
// TODO(afergs): Update to be 32 bit?
// Old status. 16-bit because upper half of 32-bit status is reserved.
...
ahci_hba_read32(void *base, uint32_t reg)
{
	return read_mmreg32((uintptr_t)base + reg);
}

static inline void
ahci_hba_write32(void *base, uint32_t reg, uint32_t val)
{
	write_mmreg32((uintptr_t)base + reg, val);
}

static inline uint32_t
ahci_port_read32(void *aport, uint32_t reg)
{
	return read_mmreg32((uintptr_t)aport + reg);
}

static inline void
ahci_port_write32(void *aport, uint32_t reg, uint32_t val)
{
...
static inline uint32_t
ahci_list_read32(void *alist, uint32_t reg)
{
	return read_mmreg32((uintptr_t)alist + reg);
}

static inline void
ahci_list_write32(void *alist, uint32_t reg, uint32_t val)
{
	write_mmreg32((uintptr_t)alist + reg, val);
}

static inline uint32_t
ahci_prdt_read32(void *aprdt, uint32_t reg)
{
	return read_mmreg32((uintptr_t)aprdt + reg);
}

static inline void
ahci_prdt_write32(void *aprdt, uint32_t reg, uint32_t val)
{
	write_mmreg32((uintptr_t)aprdt + reg, val);
}

static inline uint8_t
ahci_cfis_read8(void *acfis, uint8_t offset)
{
	return read_mmreg8((uintptr_t)acfis + offset);
}

static inline void
ahci_cfis_write8(void *acfis, uint8_t offset, uint8_t val)
{
	write_mmreg8((uintptr_t)acfis + offset, val);
}

static uint32_t
set_bit32(void *base, uint32_t offset, uint32_t mask)
{
...
static uint32_t
clear_bit32(void *base, uint32_t offset, uint32_t mask)
{
	uint32_t value = read_mmreg32((uintptr_t)base + offset);
	value &= ~mask;
	write_mmreg32((uintptr_t)base + offset, value);
...
static uint8_t
set_bit8(void *base, uint32_t offset, uint8_t mask)
{
...
static uint8_t
clear_bit8(void *base, uint32_t offset, uint8_t mask)
{
	uint8_t value = read_mmreg8((uintptr_t)base + offset);
	value &= ~mask;
	write_mmreg8((uintptr_t)base + offset, value);
...
static int ahciwait(struct aportc *c, int ms)
{
	...
	ahci_port_write32(p, PORT_CI, 1);
	as.p = p;
	as.i = 1;
	aesleep(c->pm, &as, ms);
	tfd = ahci_port_read32(p, PORT_TFD);
	if((tfd & 0x81) == 0  // Not busy and not error
			&& ahci_port_read32(p, PORT_CI) == 0) // 1:p->task 2:p->ci
		return 0;
	printd("ahci: %s: TIMEOUT", __func__);
...
static void listsetup(struct aportc *pc, int flags)
{
	...
	printd("ahci: %s: CTAB physical address=0x%08x:0x%08x\n",
			__func__,
			paddr_high32(pc->pm->ctab),
			paddr_low32(pc->pm->ctab));
}
...
static int ahciportreset(struct aportc *c)
{
	...
	udelay(1000 * 1000);
	clear_bit32(port, PORT_SCTL, 7); // TODO(afergs): Make sure PxCMD.ST == 0 first?
	return 0;
}
...
static int smart(struct aportc *pc, int n)
{
	...
	ahci_cfis_write8(cfis, 3, 0xd8 + n); /* able smart */ //afergs: Enable smart?
	ahci_cfis_write8(cfis, 5, 0x4f);
	ahci_cfis_write8(cfis, 6, 0xc2);
	listsetup(pc, Lwrite);
	// TODO(afergs): Replace 1|32 with constants
	if (ahciwait(pc, 1000) == -1 || ahci_port_read32(pc->p, PORT_TFD) & (1 | 32)) {
...
static int ahciflushcache(struct aportc *pc)
{
	...
	listsetup(pc, Lwrite);
	if (ahciwait(pc, 60000) == -1 || ahci_port_read32(pc->p, PORT_TFD) & (1 | 32)) {
...
// TODO(afergs): Change id to fis or something like that, id misleading
static int ahciidentify0(struct aportc *pc, void *id, int atapi)
{
	...
	// TODO(afergs): Investigate why this is here!
	listsetup(pc, 1<<16);  // List setup, D2H, PRDT len 1, CFIS len 5

	memset(id, 0, 0x200); /* Fill 512 bytes with 0's */
	prdt = pc->pm->ctab + ACTAB_PRDT; // TODO: More than one PRDT
...
static int64_t ahciidentify(struct aportc *pc, uint16_t *id)
{
	...
	pm->feat = 0;
	pm->smart = 0;
	i = 0; // TODO(afergs): Figure out what i is and give it a better name.
	sig = ahci_port_read32(pc->p, PORT_SIG) >> 16;
	if (sig == 0xeb14) { // TODO(afergs): Name const.
		pm->feat |= Datapi;
		i = 1;
	}
	...
	// Check if 48-bit address space is supported
	i = gbit16(id + 83) | gbit16(id + 86);
	if (i & (1 << 10)) {
		pm->feat |= Dllba;
		s = gbit64(id + 100);  // Number of user addressable logical sectors
	} else
		s = gbit32(id + 60);   // TODO(afergs): figure out what this is getting
...
static void *
malign(int amt, int align)
{
	static uint8_t lowmem[65536];
	static void *alloc = lowmem;
	void *ret;

	/* round to align */
	alloc = (void *) ((((uint64_t) alloc) + align - 1) & ~(align - 1));
	ret = alloc;
	alloc += amt;
	return ret;
}
...
static void ahciwakeup(void *p)
{
	uint16_t s; // TODO(afergs): Make 32-bit?
	uint32_t sctl;

	s = ahci_port_read32(p, PORT_SSTS);
	...
	ahci_port_write32(p, PORT_SCTL, 3 * Aipm | 0 * Aspd | Adet);
	udelay(1000 * 1000);
	clear_bit32(p, PORT_SCTL, 7); // TODO(afergs): Name const.
...
static int ahciconfigdrive(struct drive *d)
{
	...
	if (pm->list == NULL) { //== 0
		...
		// TODO(afergs): Don't fix size of command table to 1
		// TODO(afergs): Make sizing less hacks below
	...
	printd("ahci: %s: PORT_FB  physical address=0x%08x:0x%08x\n",
		__func__,
		paddr_high32(pm->fis.base),
		paddr_low32(pm->fis.base));
	...
	ahci_port_write32(port, PORT_SCTL, (3 * Aipm) | (d->mode * Aspd) | (0 * Adet));
...
static int countbits(uint32_t u) // TODO(afergs): There are better ways of doing this.
{
	int n;
...
#if 0
// TODO: Fix HBA access before use
static int
...
static uint8_t hack[1024]; // TOOD(afergs): Use hacked malign instead?
static int identify(struct drive *d)
{
	...
	if (d->info == NULL) {
		...
		d->info = (uint16_t *)hack; //kzmalloc(d->infosz, 0);
		printd("ahci: %s: HBA physical address (hack): 0x%08x:0x%08x\n", __func__,
		    paddr_high32(d->info), paddr_low32(d->info));
	}
	if (d->info == NULL) {
		d->info = d->tinyinfo;
...
static void clearci(void *p)
{
	...
	cmd = ahci_port_read32(p, PORT_CMD);
	if (cmd & Ast) {
		//ahci_port_write32(p, PORT_CMD, cmd & ~Ast);
		//ahci_port_write32(p, PORT_CMD, cmd);
...
static void updatedrive(struct drive *d)
{
	...
	ewake = 0;
	port = d->port;
	// TODO(afergs): Figure out why cause gets written back to IS
	...
	if (cause & (Aprcs | Aifs)) {
		s0 = d->state;
		// TODO(afergs): Read sstatus again?
...
static void resetdisk(struct drive *d)
{
	...
	qlock(&d->portm.ql);
	// TODO(afergs): Read cmd again? Start could have changed but I doubt it.
...
static int newdrive(struct drive *d)
{
	...
	if (ahci_port_read32(d->port, PORT_TFD) == ASbsy)
		return -1;
	qlock(&pc->pm->ql);
	if (setudmamode(pc, 5) == -1) {
...
{
	// TODO(afergs): Upper 32-bit of sstatus is reserved, but make 32-bit?
	uint16_t s;
	uint32_t task;
	char *name;
	...
	case Dnew:
		...
		case Intactive | Devphycomm | Devpresent:
			...
			// TODO(afergs): Replace 0xeb14 with constant
			s = (unsigned char)task;
			if (s == 0x7f || ((ahci_port_read32(d->port, PORT_SIG) >> 16) != 0xeb14 &&
			    (s & ~0x17) != (1 << 6)))
				break;
			spin_unlock_irqsave(&d->Lock);
			newdrive(d);
...
/* returns locked list! */
static void *ahcibuild(struct drive *d, unsigned char *cmd, void *data, int n, int64_t lba)
{
	void *cfis, *list, *prdt, *ctab; // TODO(afergs): More than one command table!
	...
	list = pm->list;
	ctab = pm->ctab;
	cfis = ctab;  // TODO(afergs): More than one command table

	// TODO(afergs): Use cfissetup(c) instead
	...
	// TODO(afergs): Remove all zero writes to a properly initialized CFIS.
	...
	prdt = ctab + ACTAB_PRDT;  // TODO: More than one PRDT.
	...
	ahci_prdt_write32(prdt, APRDT_COUNT, 1 << 31 | (d->unit->secsize * n - 2) | 1);
...
static void *ahcibuildpkt(struct aportm *pm, struct sdreq *r, void *data, int n)
{
	...
	// TODO(afergs): Remove this and initialize properly
	for (i = 0; i < 12; i++)
	...
	ahci_prdt_write32(prdt, APRDT_DBA, paddr_low32(data)); // ctab???
	ahci_prdt_write32(prdt, APRDT_DBAHI, paddr_high32(data)); // ctab???
	// TODO(afergs): Add constants
...
retry:
	...
	if ((task & (Efatal << 8)) || ((task & (ASbsy | ASdrq)) && (d->state == Dready))) {
		...
		ahcirecover(&d->portc);
		task = ahci_port_read32(port, PORT_TFD);
		// TODO(afergs): Make sure it's OK that this flag does not get written back.
	...
	if (flag & Ferror) {
		if ((task & Eidnf) == 0) // TODO(afergs): This comparison could be a mistake!
...
{
	ERRSTACK(2);
	int i, n, count, try, max, flag, task; // TODO(afergs): uint32_t?
	int64_t lba;
	char *name;
	unsigned char *cmd, *data;
	void *port;
...
{
	// TODO(afergs): Investigate non-hex
...
{
	uint32_t *p = (uint32_t *)c->mmio; /* Pointer to HBA */
...
static int
// afergs: Gets device id class of pci device
// id 1 is for storage devices
...
iarctl(struct sdunit *u, char *p, int l)
{
	...
	serror = ahci_port_read32(o, PORT_SERR);
	task = ahci_port_read32(o, PORT_TFD);
	cmd = ahci_port_read32(o, PORT_CMD);
	ci = ahci_port_read32(o, PORT_CI);
	is = ahci_port_read32(o, PORT_IS);
	sig = ahci_port_read32(o, PORT_SIG);
	sstatus= ahci_port_read32(o, PORT_SSTS);
	serrstr(serror, buf, buf + sizeof buf - 1);
	p = seprintf(p, e, "reg\ttask %#lx cmd %#lx serr %#lx %s ci %#lx is %#lx; sig %#lx sstatus %06#lx\n",
	             task, cmd, serror, buf,
	             ci, is, sig, sstatus);
	if (d->unit == NULL)
...
static void
forcemode(struct drive *d, char *mode)
{
	int i;
...
static void
runsmartable(struct drive *d, int i)
{
	ERRSTACK(2);
	if (waserror()) {
		qunlock(&d->portm.ql);
...
static void
forcestate(struct drive *d, char *state)
{
	int i;
...
{
	uint32_t cap, ghc, is, pi, ver;
	char pr[25];
	void *hba;
	struct ctlr *ctlr;

#define has(x, str) \
	do { if (cap & (x)) p = seprintf(p, e, "%s ", (str)); } while (0)
	...
	is  = ahci_hba_read32(hba, HBA_ISR);
